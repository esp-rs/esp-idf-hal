...
pub struct EspMutex<T>(*mut pthread_mutex_t, UnsafeCell<T>);
...
impl<T> EspMutex<T> {
    ...
    pub const fn new(data: T) -> Self {
        Self(PTHREAD_MUTEX_INITIALIZER as *mut _, UnsafeCell::new(data))
    }
}

impl<T> Drop for EspMutex<T> {
    fn drop(&mut self) {
        let r = unsafe { pthread_mutex_destroy(self.0) };
        debug_assert_eq!(r, 0);
    }
}
...
impl<T> mutex_trait::Mutex for EspMutex<T> {
    ...
    fn lock<R>(&mut self, f: impl FnOnce(&mut Self::Data) -> R) -> R {
        let r = unsafe { pthread_mutex_lock(self.0) };
        debug_assert_eq!(r, 0);

        let result = f(self.1.get_mut());

        let r = unsafe { pthread_mutex_unlock(self.0) };
        debug_assert_eq!(r, 0);
