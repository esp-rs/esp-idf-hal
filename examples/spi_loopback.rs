...
fn main() -> anyhow::Result<()> {
    ...
    let mut spi =
        SpiMasterDriver::new::<SPI2>(spi, sclk, serial_out, Some(serial_in), Dma::Disabled)?;

    let config_1 = config::Config::new().baudrate(26.MHz().into());
    let mut device_1 = EspSpiDevice::new(&spi, cs_1, &config_2)?;

    let config_2 = config::Config::new().baudrate(13.MHz().into());
    let mut device_2 = EspSpiDevice::new(&spi, cs_2, &config_2)?;
